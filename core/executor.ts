...
async function execCmd(name: string, cmd: string, args: string[], results: TestResult[]) {
...
async function runRust(targets: TestTarget[], results: TestResult[]) {
  if (!(await hasCmd("cargo"))) {
    for (const t of targets) results.push({ name: t.path, status: "skip", error: "cargo not found", duration: 0 })
    return
  }
